...
void executeTemotoAction()
{
  getInputParameters();
  cmi_.initialize();
  cmi_.startComponent(in_param_component_type);

  auto component_list = cmi_.listComponents(in_param_component_type);
  std::string image_topic = "camera/image";

  std::string publish_image_topic = "vaultbot/photo";
  image_pub_ = nh_.advertise<sensor_msgs::CompressedImage>(publish_image_topic, 1);
  ...
  if (component_list.local_components.size()>0)
  {
    ...
    image_sub = nh_.subscribe(image_topic, 1, &TaTakePhoto::imageCallback, this);
  ...
  {
    TEMOTO_INFO_STREAM("Component description not found: " << in_param_component_type << std::endl);  
  }
...
{
  in_param_component_type = GET_PARAMETER("component_type", std::string);
}
...
std::string in_param_component_type;
